...
test('Exemplo 1: Compra de chantily com café', () => {
...
test('Exemplo 2: Compra de chantily sem café', () => {
  const formaDePagamento = 'debito';
  const itens = ['chantily,1'];
  const resultado = caixa.calcularValorDaCompra(formaDePagamento, itens);
  expect(resultado).toBe('R$ 4,50');
});

test('Exemplo 3: Compra de combo e dois cafés', () => {
  const formaDePagamento = 'credito';
  const itens = ['combo1,1', 'cafe,2'];
  const resultado = caixa.calcularValorDaCompra(formaDePagamento, itens);
  expect(resultado).toBe('R$ 15,96');
});
